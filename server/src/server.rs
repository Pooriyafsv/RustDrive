...
use common::models::{User,FileMetadata};
use crate::auth;
                ...
                match data {
                    ...
                    Request::List { session_id } =>{
                        ...
                        let files ={
                            ...
                            file.extend_from_slice(&manager.public_files().await);
                            file
                        };
                    ...
                    Request::Search { session_id, query } => {
                        ...
                        let result ={
                            let manager = metadata.lock().await;
                            manager.search(&query, &username).await
                        };
                            ...
                            match file {
                                Some(file_find) => {
                                    ...
                                    if acccsess_file == access_int {
                                        let res = Response::Error { message: "its alearedy Private".to_string() };
                                        send_json(&mut writer, &res).await.unwrap();
                                        continue;
                                    }
                                    manager.update_access(&filename, &username, access);
                                    let res = Response::Success { message: "access changed".to_string() };
                                    send_json(&mut writer, &res).await.unwrap();
